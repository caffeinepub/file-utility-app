...
export function SmartCompressor({ onLimitReached }: SmartCompressorProps) {
  ...
  const totalSaved = fileStates
    ...
    .reduce(
      (acc, s) => acc + (s.result!.originalSize - s.result!.compressedSize),
      0,
    );
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
              ...
              <Switch
                ...
                onCheckedChange={(v) =>
                  setOptions((o) => ({ ...o, mode: v ? "lossy" : "lossless" }))
                }
              />
              <Zap
                className={cn(
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() =>
                  setOptions((o) => ({ ...o, targetType: "percentage" }))
                }
                className={cn(
              ...
              <button
                type="button"
                onClick={() =>
                  setOptions((o) => ({ ...o, targetType: "maxKB" }))
                }
                className={cn(
                  "flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all border",
            ...
            <div className="space-y-3 sm:col-span-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-semibold text-teal">
                  {options.percentage}% smaller
                </span>
              </div>
              <Slider
                value={[options.percentage]}
                onValueChange={([v]) =>
                  setOptions((o) => ({ ...o, percentage: v }))
                }
                min={10}
                max={90}
                step={5}
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setOptions((o) => ({
                      ...o,
                      maxKB: Math.max(10, Number(e.target.value)),
                    }))
                  }
        ...
        <GlassCard className="p-5">
            ...
            <div className="flex gap-2">
              {doneCount > 0 && (
                <button
                  type="button"
                  onClick={downloadAll}
                  className="btn-ghost-teal px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5"
                >
              ...
              <button
                type="button"
                onClick={clearAll}
                className="px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5 bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/8 transition-all"
              >
          ...
          {doneCount > 0 && (
            <div className="mb-4 p-3 rounded-lg bg-teal/10 border border-teal/20 flex items-center gap-3">
              ...
              <span className="text-sm text-teal">
                Saved <strong>{formatBytes(totalSaved)}</strong> across{" "}
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto scrollbar-thin pr-1">
            {fileStates.map((state) => {
              ...
              const ratio = state.result
                ? Math.round(
                    (1 -
                      state.result.compressedSize / state.result.originalSize) *
                      100,
                  )
              ...
              return (
                <div
                  key={state.id}
                  className="file-item p-3 flex items-center gap-3"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span
                        className={state.status === "done" ? "text-teal" : ""}
                      >
          ...
          <div className="mt-4 pt-4 border-t border-white/10">
            <button
              ...
              onClick={compressAll}
              disabled={
                isCompressing || fileStates.length === 0 || limitReached
              }
              className={cn(
