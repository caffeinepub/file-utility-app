...
export function UniversalConverter({
  ...
  const downloadAll = useCallback(() => {
    for (const s of fileStates) {
      if (s.results) {
        for (const r of s.results) downloadResult(r);
      }
    }
      ...
      <GlassCard className="p-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 space-y-2">
            ...
            <Select
              value={fromFormat}
              onValueChange={(v) => setFromFormat(v as FileFormat)}
            >
                  ...
                  {state.status === "done" && state.results && (
                    <div className="flex gap-1">
                      {state.results.map((r, i) => (
                        <button
                          // biome-ignore lint/suspicious/noArrayIndexKey: order-stable result list
                          key={i}
                          type="button"
